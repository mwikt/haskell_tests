...
putPlanet 1 tm (size,period,distance,color',name) = do
  ...
  preservingMatrix $ do
    ...
    renderPrimitive TriangleFan $  mapM_ (\(x,y,z) -> vertex$Vertex3 x y z ) $ kula size
...
--  translate  $ Vector3 (toGLf $ distance*(sin (tm/period))) (toGLf $ distance*(cos (tm/period))) 0.0
  renderPrimitive TriangleFan $  mapM_ (\(x,y,z) -> vertex$Vertex3 (x+a) (y+b) (z+c) ) $ kula size
